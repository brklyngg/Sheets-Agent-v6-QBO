...
function analyzeQueryIntentBasic(query, conversationHistory = []) {
  ...
  if (!intent.filters.startDate && !intent.filters.endDate) {
    ...
    console.log(`🔍 Basic analyzer current date detected as: ${today.toISOString()}`);
    ...
    if (monthYearMatch) {
      ...
      console.log(`🔍 Detected specific month and year: ${monthName} ${year} (month index: ${monthIndex})`);
      ...
      console.log(`🔍 Set date range for ${monthName} ${year}: ${intent.filters.startDate} to ${intent.filters.endDate}`);
    }
      ...
      console.log(`🔍 Basic analyzer calculated last month as: ${startDate} to ${endDate}`);
      ...
      console.log(`🔍 Basic analyzer calculated this month as: ${intent.filters.startDate} to ${intent.filters.endDate}`);
      ...
      console.log(`🔍 Basic analyzer calculated last quarter as: ${intent.filters.startDate} to ${intent.filters.endDate}`);
      ...
      console.log(`🔍 Basic analyzer calculated year to date as: ${intent.filters.startDate} to ${intent.filters.endDate}`);
      ...
      console.log(`🔍 Basic analyzer calculated last year as: ${intent.filters.startDate} to ${intent.filters.endDate}`);
    } else {
      ...
      if (yearMatch) {
        ...
        console.log(`🔍 Basic analyzer found year ${year} and set date range: ${intent.filters.startDate} to ${intent.filters.endDate}`);
      } else {
        ...
        console.log(`🔍 Basic analyzer defaulted to current year: ${intent.filters.startDate} to ${intent.filters.endDate}`);
      }
    }
  }
  ...
  if (text.includes('into') || text.includes('to sheet') || text.includes('in sheet')) {
    ...
    if (sheetMatch && sheetMatch[1]) {
      ...
      console.log(`🔍 Basic analyzer detected destination sheet: ${intent.destination}`);
    }
  }
  
  // NEW: Extract requested column list, e.g., "with columns for A, B, C" or "columns: A, B, C"
  if (text.includes('column')) {
    const colRegex = /columns?\s*(?:for|of|:)\s*([a-z0-9 ,]+)/i;
    const colMatch = text.match(colRegex);
    if (colMatch && colMatch[1]) {
      const cols = colMatch[1].split(',').map(c => c.trim()).filter(c => c.length > 0);
      if (cols.length) {
        intent.parameters.columns = cols;
        console.log('🔍 Extracted requested columns:', JSON.stringify(cols));
      }
    }
  }
  ...
  if (intent.entity === 'ProfitAndLoss') {
    ...
    if (wantsMonthly) {
      ...
      console.log('🔍 Basic analyzer detected request for monthly P&L columns');
    }
  }
...
function executeIntent(intent) {
  ...
  if (intent.type === 'unknown') {
    ...
    logRawApiResponseToActionLog(response);
    return response;
  }
  ...
  switch (intent.type) {
    case 'fetch':
      ...
      if (intent.action === 'report') {
        response = fetchReport(intent);
        logRawApiResponseToActionLog(response);
        return response;
      } else if (intent.action === 'query' || intent.action === 'entity') {
        response = fetchEntityData(intent);
        logRawApiResponseToActionLog(response);
        return response;
      } else if (intent.action === 'cell') {
        response = getCellValue(intent.parameters.cellReference);
        logRawApiResponseToActionLog(response);
        return response;
      }
      break;
    case 'create':
      if (intent.action === 'createSheet') {
        response = createNewSheet(intent);
        logRawApiResponseToActionLog(response);
        return response;
      }
      break;
    case 'modify':
      response = modifySheet(intent);
      logRawApiResponseToActionLog(response);
      return response;
    case 'help':
      response = {
      ...
      return response;
    case 'diagnostic':
      response = runQuickBooksDiagnostic();
      logRawApiResponseToActionLog(response);
      return response;
    case 'custom':
      response = executeCustomApiCall(intent);
      logRawApiResponseToActionLog(response);
      return response;
  }
...
function executeCustomApiCall(intent) {
  try {
    ...
    if (intent.qboApiCall && intent.sheetsApiCall && intent.destination) {
      ...
      // Write to the sheet
      const writeResult = writeToSheet(destinationRange, formattedData, true);
  ...
  try {
    ...
    if (intent.filters.startDate && intent.filters.endDate) {
      ...
      try {
        ...
        console.log(`🔍 Using date range: ${params.start_date} to ${params.end_date}`);
      } catch (dateError) {
    ...
    if (intent.entity === 'ProfitAndLoss') {
      ...
      console.log(`📊 Fetching P&L report with params: ${JSON.stringify(params)}`);
    }
    ...
    logRawApiResponseToActionLog(reportData);
    
    // Debug: Write raw API response to a hidden sheet for troubleshooting
    try {
      const ss = SpreadsheetApp.getActiveSpreadsheet();
      let debugSheet = ss.getSheetByName('Debug API Response');
      if (!debugSheet) {
        debugSheet = ss.insertSheet('Debug API Response');
        debugSheet.hideSheet();
      }
      debugSheet.clear();
      debugSheet.getRange(1, 1).setValue('Raw API Response (JSON)');
      debugSheet.getRange(2, 1).setValue(JSON.stringify(reportData, null, 2));
    } catch (debugErr) {
      console.error('Failed to write debug API response: ' + debugErr.message);
    }
...
function fetchEntityData(intent) {
  try {
    ...
    if (intent.filters && intent.filters.startDate && intent.filters.endDate) {
      console.log('🔍 DATE FILTER REQUESTED: ' + intent.filters.startDate + ' to ' + intent.filters.endDate);
    }
    ...
    // Map user-friendly column names to QBO fields
    const userToQboMap = {
    ...
    if (intent.parameters && intent.parameters.columns && Array.isArray(intent.parameters.columns)) {
      userColumns = intent.parameters.columns;
      // Map user columns to QBO fields if possible
      requestedColumns = userColumns.map(col => userToQboMap[col] || col);
    }
    // Default columns for Invoice
    const defaultInvoiceColumns = [
      'TxnDate', 'DueDate', 'DocNumber', 'CustomerRef', 'TotalAmt', 'Balance', 'Id'
    ];
    // Default columns for Bill
    const defaultBillColumns = [
      'TxnDate', 'DueDate', 'DocNumber', 'VendorRef', 'TotalAmt', 'Balance', 'Id'
    ];
    // Build query - only select mapped fields if user columns specified
    let query = '';
    if (intent.entity === 'Bill') {
      const cols = requestedColumns || defaultBillColumns;
      query = `SELECT ${cols.join(', ')} FROM ${intent.entity}`;
    } else if (intent.entity === 'Invoice') {
      const cols = requestedColumns || defaultInvoiceColumns;
      query = `SELECT ${cols.join(', ')} FROM ${intent.entity}`;
    } else {
    ...
    if (intent.filters && intent.filters.startDate && intent.filters.endDate) {
      if (intent.entity === 'Invoice' || intent.entity === 'Bill' || intent.entity === 'Purchase') {
        ...
        console.log(`🚨 Date filter applied in SQL query: ${startDate} to ${endDate}`);
      }
    }
    ...
    logRawApiResponseToActionLog(queryData);
    ...
    if (intent.entity === 'Bill' && queryData && queryData.QueryResponse && queryData.QueryResponse.Bill) {
      console.log('🔍 APPLYING STRICT DATE FILTERING to ensure correct date range');
      ...
      if (intent.filters && intent.filters.startDate && intent.filters.endDate) {
        ...
        console.log(`🔎 Filtering for bills between ${startDateObj.toISOString()} and ${endDateObj.toISOString()}`);
        console.log(`🔎 Original bill count: ${queryData.QueryResponse.Bill.length}`);
        ...
        console.log(`🔎 FILTERED FROM ${queryData.QueryResponse.Bill.length} TO ${filteredBills.length} BILLS`);
        ...
        if (filteredBills.length === 0) {
          console.warn(`⚠️ WARNING: All bills were filtered out. This may indicate a problem with the date range.`);
          console.warn(`⚠️ Requested date range: ${intent.filters.startDate} to ${intent.filters.endDate}`);
        }
      }
    }
    ...
    let formattedData;
    if (intent.entity === 'Bill') {
      formattedData = formatBillData(queryData, requestedColumns);
    } else if (intent.entity === 'Invoice') {
      formattedData = formatInvoiceData(queryData, requestedColumns, userColumns);
    } else {
      formattedData = formatDataForSheet(queryData, 'query');
    }
    if (formattedData && formattedData[0]) {
      console.log('OUTPUT COLUMNS:', formattedData[0]);
    }
 ...
 */
function formatBillData(queryData, requestedColumns) {
...
/**
 * Formats invoice data in a more user-friendly way.
 * 
 * @param {Object} queryData - The raw query response from QuickBooks
 * @return {Array} 2D array with formatted invoice data
 */
function formatInvoiceData(queryData, requestedColumns, userColumns) {
  if (!queryData || !queryData.QueryResponse || !queryData.QueryResponse.Invoice) {
    return [['No invoice data found']];
  }
  const invoices = queryData.QueryResponse.Invoice;
  // Default headers and mapping
  const allHeaders = [
    { key: 'TxnDate', label: 'Invoice Date', format: v => v ? new Date(v).toLocaleDateString() : '' },
    { key: 'DueDate', label: 'Invoice Due Date', format: v => v ? new Date(v).toLocaleDateString() : '' },
    { key: 'DocNumber', label: 'Invoice Number' },
    { key: 'CustomerRef', label: 'Customer Name', format: v => v && v.name ? v.name : 'Unknown' },
    { key: 'TotalAmt', label: 'Amount', format: v => v !== undefined ? ('$' + parseFloat(v).toFixed(2)) : '' },
    { key: 'Balance', label: 'Balance', format: v => v !== undefined ? ('$' + parseFloat(v).toFixed(2)) : '' },
    { key: 'Status', label: 'Status', format: (v, inv) => {
      if (inv.Balance === 0) return 'Paid';
      if (inv.Balance > 0 && inv.Balance === inv.TotalAmt) return 'Unpaid';
      if (inv.Balance > 0 && inv.Balance < inv.TotalAmt) return 'Partially Paid';
      return 'Unknown';
    } },
    { key: 'Id', label: 'Invoice ID' },
    { key: 'Line', label: 'Line Items', format: v => Array.isArray(v) ? v.map(l => l.Description || '').join('; ') : '' }
  ];
  // If user specified columns, filter headers by userColumns (for nice header order)
  let headersToUse = allHeaders;
  if (userColumns && Array.isArray(userColumns)) {
    headersToUse = userColumns.map(col => {
      // Try to find by label (case-insensitive)
      const found = allHeaders.find(h => h.label.toLowerCase() === col.toLowerCase() || h.key.toLowerCase() === col.toLowerCase());
      return found || { key: col, label: col };
    });
  } else if (requestedColumns && Array.isArray(requestedColumns)) {
    headersToUse = allHeaders.filter(h => requestedColumns.includes(h.key) || requestedColumns.includes(h.label));
  } else {
    // Default: only the minimum necessary columns
    headersToUse = allHeaders.filter(h => ['TxnDate','DueDate','DocNumber','CustomerRef','TotalAmt','Balance','Status','Id'].includes(h.key));
  }
  const headers = headersToUse.map(h => h.label);
  const rows = invoices.map(inv => {
    return headersToUse.map(h => {
      if (h.format) return h.format(inv[h.key], inv);
      // Special: for custom keys, try to output as string
      return inv[h.key] !== undefined ? (typeof inv[h.key] === 'object' ? JSON.stringify(inv[h.key]) : inv[h.key]) : '';
    });
  });
  return [headers, ...rows];
}
...
function processProfitAndLossReport(reportData, intent, params) {
  console.log('🔍 Using enhanced P&L report processing');
  // Reference for finance language: https://www.golimelight.com/fpa-glossary
  try {
    ...
    if (reportData.Columns && reportData.Columns.Column) {
      isMonthlyReport = reportData.Columns.Column.length > 2 && 
        reportData.Columns.Column.some(col => col.ColTitle && (col.ColTitle.includes('Jan') || col.ColTitle.includes('Feb') || col.ColTitle.includes('Mar')));
    }
    // Date range fix: always use first and last day of requested month
    ...
    if (formattedData.length > 0 && startDate && endDate) {
      // Format dates for display
      const displayStart = new Date(startDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
      const displayEnd = new Date(endDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
      formattedData.unshift(['', `Profit & Loss: ${displayStart} to ${displayEnd}`]);
    }
    // Determine sheet name
    const sheetName = intent.destination || `ProfitAndLoss_${startDate}`;
    // Write the data to a sheet
    const writeResult = writeToSheet(sheetName, formattedData, true);
    if (writeResult.success) {
      // Natural language, finance-style response
      const monthName = new Date(startDate).toLocaleString('default', { month: 'long' });
      const year = new Date(startDate).getFullYear();
      let message = `Here's your ${monthName} ${year} P&L (${startDate}–${endDate}). Let me know if you need a breakdown by department, a comparison to budget, or anything else.`;
      if (isMonthlyReport) {
...
function processWithOpenAI(query, conversationHistory, apiCallList) {
  try {
    ...
    console.log(`🧠 Processing with OpenAI using current date: ${currentDate}`);
...
If the user's query is ambiguous or lacks necessary information, specify what additional information is needed.`;
    ...
    try {
      ...
      if (intentMatch) {
        ...
        if (intent.filters && (intent.filters.startDate || intent.filters.endDate)) {
          console.log(`🔍 OpenAI resolved date filters: startDate=${intent.filters.startDate}, endDate=${intent.filters.endDate}`);
          
          // Validate date formats
          if (intent.filters.startDate && !/^\d{4}-\d{2}-\d{2}$/.test(intent.filters.startDate)) {
            console.warn(`⚠️ Invalid startDate format: ${intent.filters.startDate}, correcting it`);
            ...
            try {
              ...
              console.log(`✓ Corrected to: ${fixedDate}`);
            } catch (e) {
          ...
          if (intent.filters.endDate && !/^\d{4}-\d{2}-\d{2}$/.test(intent.filters.endDate)) {
            console.warn(`⚠️ Invalid endDate format: ${intent.filters.endDate}, correcting it`);
            // Try to parse and fix the date
            try {
...
function logRawApiResponseToActionLog(apiResponse) {
  try {
    // Test log entry
    logAction('TEST', 'Raw API Response', 'This is a test');
    // Simple JSON log entry
    logAction('TEST', 'Raw API Response', JSON.stringify({test: true, time: new Date().toISOString()}));
    // Actual API response log entry
    logAction('Raw API Response', 'JSON', JSON.stringify(apiResponse));
  } catch (e) {
    logAction('Raw API Response', 'Error logging JSON', e.message);
  }
}
